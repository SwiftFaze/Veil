...
- **Always put `Closes #N` in the PR body.** GitHub's own auto-close never fires
  ...
  only if the reference is there.
...
- **Step 7.5 — close the linked issue.** Automatic on merge *if* the PR body
  says `Closes #N` (see "Never do these" above). Still manual for an issue with
  no PR of its own — a `brainstorm-issue`/`brainstorm-milestone` item finished
  as part of other work: `gh issue close <n> --repo SwiftFaze/Veil --reason
  completed`. If the VEIL board's `Status` doesn't follow to Done on its own,
  set it explicitly.
