...
| `sandbox-spawn-edit.feature` | Dev-only "Player" provider (sandbox-dev-console.feature's second provider) that attaches to and live-edits the actual running player's stats/class via a dev-only in-game keybind, rather than spawning a disconnected copy; `Main.java`'s composition-root wiring and the dev-only build-gating mechanism are non-goals, verified by manual playtest instead |
